                        ...
                        <div class="col col-custom mx-2 px-0  background-color-1">
                            <div class="col col-custom-header"><p class="mb-0">Staff</p></div>
                                <div class="col col-staff background-color-1">
                                    <select id="staffDropdown" class="form-control" name="staff" style="width: 100%;" required>
                                        <option value=""></option>
                                            <option 
                                                value="{{$user->id}}" 
                ...
                <div class="card-header for-customer d-flex  justify-content-center p-0 mb-2">
                    <div class="row no-gutters full-height full-width p-0 ">
                        <div class="col-8 px-0 d-flex">
                            <input type="text" style="height: -webkit-fill-available;" class="full-height full-width" name="customer" id="customer" disabled>
                        </div>
                        <div class="col-4  px-0">
                            <button type="button" style="height: -webkit-fill-available;" class="full-height full-width for-customer-button" data-bs-toggle="modal" data-bs-target="#addClient">add customer</button>
                        </div>
                    </div>
                </div>
                <div class="card-header for-pay d-flex  justify-content-center p-0 mb-2">
                    <div class="row no-gutters full-height full-width p-0 ">
                        <div class="col col-custom px-0">
                            <button type="button" class="full-height full-width for-customer-button for-pay-button" id="pay-button">PAY</button>
                        </div>
                    </div>
                </div>
                <div class="card-header for-sales d-flex  justify-content-center p-0 mb-2">
                    <div class="row no-gutters full-height full-width p-0 ">
                        <div class="col col-custom me-2 px-0">
                            <button type="button" onclick="newSale()" class="full-height full-width for-customer-button for-pay-button">New Sale</button>
                        </div>
    ...
    <div class="modal-dialog modal-dialog-scrollable modal-fullscreen">
        <div class="modal-content">
            ...
            <div class="modal-body">
                    <div class="row">
                        <div class="col-6">
                            <p style='font-weight: 500 !important; color:black;'>Other Method</p>
                            <div id="snap-container" class="full-width"></div>
                        </div>
                        <div class="col-6">
                            <p style='font-weight: 500 !important; color:black;'>Cash Method</p>
                            <label for="total-pay">Balance Total</label>
                            <div class="input-group input-group-sm">
                                <span class="input-group-text" style='background-color: rgba(255, 255, 255, 0); border-color: transparent; font-weight: 500 !important; color:black;' id="basic-addon2">Rp</span>
                                <input type="tel" aria-describedby="basic-addon2" style="background-color: rgba(255, 255, 255, 0); border-color: transparent; font-weight: 500 !important; color:black;" class="form-control form-control-sm @error('date') is-invalid @enderror" id="total-pay" disabled>
                            </div>
                            <label for="total-pay" class="mb-1">Paying Amount</label>
                            <div class="mb-2">
                                <div class="input-group input-group-sm">
                                    <span class="input-group-text" style='background-color: #DBF2FC !important; font-weight: 500 !important; color:black;' id="basic-addon2">Rp</span>
                                    <input type="tel" aria-describedby="basic-addon2" style="background-color: #DBF2FC !important; font-weight: 500 !important; color:black;" class="form-control form-control-sm @error('date') is-invalid @enderror" name="paying_amount" id="paying_amount">
                                </div>
                                <small id="unenough" class="text-danger font-italic">Payment is unenough</small>
                            </div>
                            <label for="total-pay">Change Return</label>
                            <div class="input-group input-group-sm">
                                <span class="input-group-text" style='background-color: rgba(255, 255, 255, 0); border-color: transparent; font-weight: 500 !important; color:black;' id="basic-addon2">Rp</span>
                                <input type="tel" aria-describedby="basic-addon2" style="background-color: rgba(255, 255, 255, 0); border-color: transparent; font-weight: 500 !important; color:black;" class="form-control form-control-sm @error('date') is-invalid @enderror" id="change_return" name="change_return" disabled>
                            </div>
                            <div class=""style="float:right">
                                <button type="button" id="pay-cash" class="btn btn-primary me-3">Pay Cash</button>
                            </div>
                        </div>
                    </div>
            </div>
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.error) {
                    ...
                    document.getElementById('clientId').value = response.customer.id; 
    ...
    $(document).ready(function() {
        ...
        function setTablePayment() {
            setTimeout(function() {
                ...
                const unenough = document.getElementById('unenough');

                if (pay >= grandtotal) {
                    change_return = pay - grandtotal;
                    //tampil tombol bayar
                ...
                $('#total-pay').val(numberFormat(grandtotal));
                $('#change_return').val(numberFormat(change_return));
    ...
    function newSale() {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                var formData = $('#pos_sale').serialize();
            ...
            if (result.isConfirmed) {
                ...
                success: function(response) {
                    if (response.error) {
                        ...
                        window.location.href = `/cashier/edit/${saleId}`;
                    }
                },
                error: function (xhr, status, error) {
                    Swal.fire({
...
<script>
    $('#pay-cash').on('click', function (event) {
            event.preventDefault();
            // Pastikan pin diubah menjadi huruf besar
            saveSale();
        });
</script>
...
<script>
    $(document).ready(function() {
        ...
        $('#pay-button').click(function(e) {
            e.preventDefault();
            // Menggunakan JavaScript murni untuk memeriksa validitas
            var staffDropdown = document.getElementById('staffDropdown');

            if (!staffDropdown.checkValidity()) {
                staffDropdown.reportValidity();
                return; // Keluar dari fungsi jika ada input yang tidak valid
            }

            // Tambahkan kondisi untuk memeriksa apakah kedua objek kosong
            if (Object.keys(productsObj).length == 0 && Object.keys(productsWithVariantObj).length == 0) {
                    Swal.fire({
                        icon: 'error',
                        title: 'Oops...',
                        html: '<ol style="text-align: start">Your chart is empty</ol>',
                    });
                return; // Keluar dari fungsi jika kedua objek kosong
            }
...
<script>
    function saveSale(order_id) {
        var formData = $('#pos_sale').serialize();
        var paying_amount = $('#paying_amount').val();
        var change_return = $('#change_return').val();
        ...
        success: function(response) {
            if (response.error) {
                ...
                //ngeprint dulu                
                var fakturDiv = document.getElementById("facture");
                // Mengosongkan konten sebelumnya
                fakturDiv.innerHTML = '';

                // Membuat elemen HTML untuk faktur
                var headerDiv = document.createElement("div");
                headerDiv.className = "row";

                // Menambahkan konten ke header
                headerDiv.innerHTML = `
                        ...
                        <div class="width-full d-flex" style="
                            BORDER: DASHED;
                            border-width: thin 0px;
                        ">
                            <div class="col-1" style="width: 60%;">TANGGAL</div>
                            <div class="col" style="width: 40%;">STAF</div>
                        </div>
                `;

                // Menambahkan header ke div faktur
                fakturDiv.appendChild(headerDiv);
                console.log(response.product);

                // Menambahkan produk ke faktur
                response.products.forEach(detail => {
                    var productDiv = document.createElement("div");
                    productDiv.className = "width-full d-flex";
                    productDiv.innerHTML = `
                        <div class="col-1" style="width: 50%;">${detail.product_name}</div>
                        <div class="col-1" style="width: 10%;">${detail.quantity}</div>
                        <div class="col" style="width: 20%;">${detail.price}</div>
                        <div class="col" style="width: 20%;">${detail.total}</div>
                    `;
                    fakturDiv.appendChild(productDiv);
                });

                // Menampilkan faktur
                fakturDiv.style.display = 'block';
                // Memanggil fungsi print
                window.print();
                // Menyembunyikan faktur setelah pencetakan
...
<script>
    // Menutup pop-up ketika modal ditutup
    $('#pay').on('hidden.bs.modal', function () {
        snap.hide(); // Menutup pop-up    
    });
</script>
