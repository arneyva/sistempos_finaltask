...
return new class extends Migration
{
        ...
        if (Schema::hasTable('clients')) {
            Schema::table('clients', function (Blueprint $table) {
                if (! Schema::hasColumn($table->getTable(), 'is_poin_activated')) {
                    $table->boolean('is_poin_activated')->default(1);
                }
            });
        }
    ...
    public function down(): void
    {
        if (Schema::hasTable('clients') && Schema::hasTable('client_tiers')) {
            Schema::table('clients', function (Blueprint $table) {
                if (Schema::hasColumn($table->getTable(), 'client_tier_id')) {
                    $table->foreign('client_tier_id', 'clients_client_tier_id')->references('id')->on('users')->onUpdate('RESTRICT')->onDelete('RESTRICT');
                } else {
                    $table->integer('client_tier_id')->index('clients_client_tier_id')->nullable();
                    $table->foreign('client_tier_id', 'clients_client_tier_id')->references('id')->on('users')->onUpdate('RESTRICT')->onDelete('RESTRICT');
                }
            });
        }
